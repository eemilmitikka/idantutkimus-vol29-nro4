---
title: "\"Hyökkäyssodan kannatus Venäjällä\" -artikkelin R-koodit"
author: "Eemil Mitikka"
date: "Päivitetty: 22 joulukuuta 2022"
...
p_arvot <- "Tilastollinen merkitsevyys: *** = p < 0.001, ** = p < 0.01, * = p < 0.05"
...
# write.csv(levada_data, here("data/levada_all.csv"))
...
# write.csv(vtsiom_sotakannatus, here("data/vtsiom_all.csv"))
...
# write.csv(fom_data, here("data/fom_all.csv"))
...
# write.csv(rf_all, here("data/rf_all.csv"))
...
# write.csv(chronicles_all, here("data/chronicles_all.csv"))
...
figure_yandex <-
  ...
  labs(title = '"telegram", "vpn" ja "youtube"',
       ...
       x = toupper("Päivämäärä (kuukausi/vuosi)"),
       color = "Hakusana") +
...
# jpeg(here("img/kuva_2_paivitetty.jpg"), units = "in", width = 17, height = 12, res = 300)
# kuva_2_paivitetty
# dev.off()
...
```{r}
taulukko_1 <- 
  data.frame(survey = c("Chronicles", 
                        "Russian Field",
                        "VTsIOM"),
           date = c("14.–19.5.2022",
                    "5.–7.3.2022",
           ...
           sample_size = c(1605, 
                           1606, 
                           9600),
           state_dependent = c("Ei",
                               "Ei",
                               "Kyllä"),
           affilition = c("Oppositio",
                          "Oppositio",
                          "Presidentinhallinto, valtio"))
# Rename the columns
colnames(taulukko_1) <- c("Organisaatio",
                  "Päivämäärä",
                  "Otoskoko",
                  "Riippuvuus valtiosta",
                  "Kytkös")
# Print Table 1
knitr::kable(taulukko_1,
             caption = "Taulukko 1. Tutkimusaineistojen kuvaus.")
...
chronicles_model_3 <- svyglm(formula = support ~ age + 
                               gender +
                               tnp +
                               edu_gr +
                               tutut_ihmiset +
...
kuva_4 <-
  ...
  kuva_4.2 /
  kuva_4.3 +
    plot_annotation(caption = p_arvot) &
...
chronicles_coxsnell <- paste(expression(R^2), "=", chronicles_model_3_coxsnell,"<br>")
chronicles_nagelkerke <- paste(expression(R^2), "=", chronicles_model_3_nagelkerke,"<br>")

kuva_4_final <-
kuva_4.3 +
  theme_oma3() +
  labs(title = paste("Chronicles-aineiston malli:", odds),
       caption = c(paste(chronicles_coxsnell, 
                       chronicles_nagelkerke),
                       p_arvot)) +
  theme(plot.title = element_text(color = vari_chronicles,
                                  margin = margin(10,0,0,0)),
        plot.caption = element_markdown(margin = margin(10,0,0,0),
                                        hjust = c(1, 2.2)))
...
# jpeg(here("img/kuva_4_final.jpg"), units = "in", width = 14, height = 12, res = 300)
# kuva_4_final
# dev.off()
...
```{r}
rf <- 
  here("data/МАССИВ SPSS Russian Field - _Россия-Украина_ (всероссийский телефонный опрос, 1600, 5-7 марта).sav") %>% 
  read_spss() %>% 
  clean_names() %>% 
  mutate(support = q5,
         ika = q3,
...
rf_model_1_cis <- 
  data.frame(confint(rf_model_1)) %>% 
  rename(lower_ci = 1,
         upper_ci = 2)
# Russian Field, Model 1:  Calculate Odd Ratios
rf_model_1_or <- 
  exp(cbind(coef(rf_model_1), confint(rf_model_1))) %>% 
  as.data.frame() %>% 
  rename(oratio = 1,
         oratio_lower = 2,
         oratio_upper = 3)
...
rf_model_1_coxsnell <- 
  paste(
    round(
      100*psrsq(rf_model_1, method = "Cox-Snell"), digits = 1), "% (Cox-Snell)", sep = "")

rf_model_1_nagelkerke <- 
  paste(
    round(
      100*psrsq(
        rf_model_1,
...
# Russian Field, Model 2: subset the data
rf_model_2_data <- 
  rf %>% 
...
rf_model_2_cis <- 
  data.frame(confint(rf_model_2)) %>% 
  rename(lower_ci = 1,
         upper_ci = 2)

# Russian Field, Model 2:  Calculate Odd Ratios
rf_model_2_or <- 
  exp(cbind(coef(rf_model_2), confint(rf_model_2))) %>% 
  as.data.frame() %>% 
  rename(oratio = 1,
         oratio_lower = 2,
         oratio_upper = 3)
...
# Russian Field, Model 2: plot X-axis labels
...
# Russian Field, Model 3:  define weights
rf_model_3_design <- svydesign(ids =~ 1,
                               data = rf_model_3_data,
                               weights = rf_model_3_data$vtpv)

# Russian Field, Model 3:  run the model
rf_model_3 <- svyglm(support ~ ika + 
                       sukupuoli + 
                       tv + 
                       youtube + 
                       telegram + 
                       ...
                       sanktio_ystavat + 
                       sanktio_perhe + 
                       sanktio_ei_vaikutusta,
       design = rf_model_3_design,
       family = binomial(logit))
# Russian Field, Model 3: print the results
summary(rf_model_3)

# Russian Field, Model 3: Confidence intervals
rf_model_3_cis <- 
  data.frame(confint(rf_model_3)) %>% 
  rename(lower_ci = 1,
         upper_ci = 2)

# Russian Field, Model 3: Calculate Odd Ratios
rf_model_3_or <- 
  exp(cbind(coef(rf_model_3), confint(rf_model_3))) %>% 
  as.data.frame() %>% 
  rename(oratio = 1,
         oratio_lower = 2,
         oratio_upper = 3)

# Russian Field, Model 3: data for plot
rf_model_3_plot_data <- 
  tidy(rf_model_3) %>% 
  cbind(rf_model_3_cis, rf_model_3_or) %>% 
  slice(-1) %>%
  mutate(term = as.factor(term))

# Russian Field, Model 3:  Pseudo R^2: Cox-Snell & Nagelkerke
rf_model_3_coxsnell <- 
  paste(
    round(
      100*psrsq(
...
# Russian Field, Model 3: X-axis labels
rf_model_3_labels <- c("30–44-vuotiaat **",
                       "45–59-vuotiaat **",
                       "60-vuotiaat ja vanhemmat",
...
# Russian Field, Model 3: Odd ratios plot
kuva_5.3 <-
rf_model_3_plot_data %>% 
  ggplot(aes(x = term, y = oratio)) +
  geom_point(size = 6, color = vari_rf) +
  ...
  theme(plot.title = element_text(color = vari_rf)) +
  geom_hline(yintercept = 1, linetype = 2) +
  labs(title = paste("RUSSIAN FIELD, Malli 3:", odds),
       x = NULL,
       y = toupper("Riskiluku (logaritminen asteikko)"),
       caption = bquote(atop(paste(R^2, " = ", .(rf_model_3_coxsnell)),
                      paste(R^2, " = ", .(rf_model_3_nagelkerke))))) +
  scale_x_discrete(labels = rf_model_3_labels)
...
rf_coxsnell <- paste(expression(R^2), "=", rf_model_3_coxsnell,"<br>")
rf_nagelkerke <- paste(expression(R^2), "=", rf_model_3_nagelkerke,"<br>")

kuva_5_final <-
kuva_5.3 +
  labs(title = paste("Russian Field-aineiston malli:", odds),
       caption = c(paste(rf_coxsnell,
                       rf_nagelkerke),
                       p_arvot)) +
  theme_oma3() +
  theme(plot.title = element_text(color = vari_rf,
...
# jpeg(here("img/kuva_5_final.jpg"), units = "in", width = 14, height = 12, res = 300)
# kuva_5_final
# dev.off()
...
kuva_6.1 <- 
  ...
  ggplot(aes(x = term, y = oratio)) +
  geom_point(size = 6, color = vari_vtsiom) +
  coord_flip() +
  scale_y_log10(breaks = seq(0, 10, 1)) +
  geom_errorbar(aes(ymin = oratio_lower, ymax = oratio_upper), width = 0.2, color = "grey50") +
  theme_oma2() +
  theme(plot.title = element_text(color = vari_vtsiom)) +
  geom_hline(yintercept = 1, linetype = 2) +
  labs(title = paste("VTSIOM, Malli 1:", odds),
       x = NULL,
       y = toupper("Riskiluku (logaritminen asteikko)"),
...
vtsiom_model_full_data <- rbind(vtsiom_1_model_2, 
                             vtsiom_2_model_2,
                             vtsiom_3_model_2,
                             vtsiom_4_model_2,
                             vtsiom_5_model_2,
...
vtsiom_model_2_cis <- 
  data.frame(confint(vtsiom_model_2)) %>% 
  rename(lower_ci = 1,
         upper_ci = 2)

# VTSIOM, Model 2: Calculate Odd Ratios
vtsiom_model_2_or <- 
  exp(cbind(coef(vtsiom_model_2), confint(vtsiom_model_2))) %>% 
  as.data.frame() %>% 
  rename(oratio = 1,
         oratio_lower = 2,
         oratio_upper = 3)
...
kuva_6.2 <-
  ...
  theme(plot.title = element_text(color = vari_vtsiom)) +
  geom_hline(yintercept = 1, linetype = 2) +
  labs(title = paste("VTSIOM, Malli 2:", odds),
       x = NULL,
       y = toupper("Riskiluku (logaritminen asteikko)"),
...
# VTSIOM, Model 3: Define weights
vtsiom_model_3_design <- 
  svydesign(ids =~ 1,
            data = vtsiom_model_3_data,
            weights = vtsiom_model_3_data$weight1)

# VTSIOM, Model 3: Define model
vtsiom_model_3 <- 
  svyglm(support ~ 
           ika + 
           sukupuoli + 
           ...
           internet_kaytto + 
           perhe_tulot,
         design = vtsiom_model_3_design,
         family = binomial(logit))

# VTSIOM, Model 3: print the results
summary(vtsiom_model_3)

# VTSIOM, Model 3: Confidence intervals
vtsiom_model_3_cis <- 
  data.frame(confint(vtsiom_model_3)) %>% 
  rename(lower_ci = 1,
         upper_ci = 2)

# VTSIOM, Model 3: Calculate Odd Ratios
vtsiom_model_3_or <- 
  exp(cbind(coef(vtsiom_model_3), confint(vtsiom_model_3))) %>% 
  as.data.frame() %>% 
  rename(oratio = 1,
         oratio_lower = 2,
         oratio_upper = 3)

# VTSIOM, Model 3: data for plot
vtsiom_model_3_plot_data <- 
  tidy(vtsiom_model_3) %>% 
  cbind(vtsiom_model_3_cis, vtsiom_model_3_or) %>% 
  slice(-1) %>%
  mutate(term = as.factor(term))

# VTSIOM, Model 3: Pseudo R^2: Cox-Snell & Nagelkerke
vtsiom_model_3_coxsnell <- 
  paste(
    round(
      100*psrsq(
        vtsiom_model_3,
...
# VTSIOM, Model 3: plot labels
vtsiom_model_3_labels <- c("Kaupungin tapainen taajama",
                           "Alle 50,000 asukkaan kaupunki",
                           "50,000–100,000 asukkaan kaupunki",
                           "100,000–500,000 asukkaan kaupunki *",
                           "500,000–950,000 asukkaan kaupunki *",
...
# VTSIOM, Model 3: Odd ratios plot
kuva_6.3<- 
  vtsiom_model_3_plot_data %>% 
  ggplot(aes(x = term, y = oratio)) +
  geom_point(size = 6, color = vari_vtsiom) +
  ...
  theme(plot.title = element_text(color = vari_vtsiom)) +
  geom_hline(yintercept = 1, linetype = 2) +
  labs(title = paste("VTSIOM, Malli 3:", odds),
       x = NULL,
       y = toupper("Riskiluku (logaritminen asteikko)"),
              caption = bquote(atop(paste(R^2, " = ", .(vtsiom_model_3_coxsnell)),
                             paste(R^2, " = ", .(vtsiom_model_3_nagelkerke))))) +
  scale_x_discrete(labels = vtsiom_model_3_labels)
...
# jpeg(here("img/kuva_6.jpg"), units = "in", width = 16, height = 20, res = 300)
# kuva_6
# dev.off()

vtsiom_coxsnell <- paste(expression(R^2), "=", vtsiom_model_3_coxsnell,"<br>")
vtsiom_nagelkerke <- paste(expression(R^2), "=", vtsiom_model_3_nagelkerke,"<br>")

kuva_6_final <- 
kuva_6.3 +
  labs(title = paste("VTsIOM-aineiston malli:", odds),
       caption = c(paste(vtsiom_coxsnell,
                       vtsiom_nagelkerke),
                       p_arvot)) +
  theme_oma3() +
  theme(plot.title = element_text(color = vari_vtsiom,
...
# jpeg(here("img/kuva_6_final.jpg"), units = "in", width = 14, height = 12, res = 300)
# kuva_6_final
# dev.off()
