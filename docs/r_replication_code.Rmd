...
yandex_sota <-
  ...
  theme(text = element_text(family = "Arial", face = "bold"),
        ...
        legend.position = "bottom",
        axis.text.x = element_text(angle = 30)) +
  geom_vline(xintercept = hyokkays, linetype = "dashed", size = 1.5) +
  geom_curve(aes(x = viivanpaa, y = 6500000, xend = hyokkays, yend = 8000000),
    data = yandex_data,
    arrow = arrow(length = unit(0.03, "npc")),
    curvature = -0.3,
    color = "red") +
  annotate("text", y = 5500000, x = viivanpaa, label = "Russia's\nlarge-scale attack\nbegins", size = 5) +
  scale_x_date(date_labels = "%m/%Y") +
  labs(title = '"special operation" vs. "war"',
       y = NULL,
       x = "DATE",
       color = "KEYWORD") +
  scale_y_continuous(labels = scales::comma) +
  scale_color_manual(values = c("red", "blue"),
                     labels = c('Special operation in Ukraine\n("спецоперация на украине")', 'War in Ukraine\n("война на украине")'))



kuva_2 <- figure_yandex + figure_yandex_2 +
  plot_annotation(title = toupper('Tiedonvälitykseen ja sotaan liittyvien hakusanojen käyttö Yandexissa'), 
                  subtitle = "1.8.2020–31.7.2022") & theme(text = element_text(family = "Times", size = 20, face = "bold"))
